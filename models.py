#Modified and used Huggingface's Transformer code
...
class ViTForImageClassification(ViTPreTrainedModel):
    def __init__(self, config: ViTConfig) -> None:
        ...
        # Classifier head
        self.classifier = nn.Linear(config.hidden_size + 5, config.num_labels) if config.num_labels > 0 else nn.Identity()
    ...
    def forward(
        ...
        meta2 = meta
        final_me = torch.cat((sequence_output[:, 0, :], meta2),1)
        logits = self.classifier(final_me)
